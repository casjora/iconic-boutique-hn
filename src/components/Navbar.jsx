...
export default function Navbar() {
  const { user, cart, logout, currentView, setView, theme, toggleTheme } =
    useStore();
  ...
  const navItemClass = (view) => {
    const base =
      "inline-flex items-center gap-1 px-2 xl:px-2.5 py-1.5 text-xs font-bold rounded-xl transition-all cursor-pointer shrink-0 whitespace-nowrap ";
    if (currentView === view) {
      return (
        base +
  ...
  const mobileNavItemClass = (view) => {
    const base =
      "flex items-center gap-2.5 px-4 py-3 text-sm font-bold rounded-xl transition-all cursor-pointer ";
    if (currentView === view) {
      return (
        base +
        "bg-neutral-900 text-white dark:bg-amber-400 dark:text-neutral-950 shadow"
      );
    }
    return (
      base +
              ...
              {(isOwner || isVendedor) && (
                <>
                  ...
                  <button
                    ...
                    className={navItemClass("showroom")}
                  >
                  ...
                  <button
                    onClick={() => handleNav("barcodes")}
                    className={navItemClass("barcodes")}
                  >
      ...
      {mobileMenuOpen && (
        <div className="lg:hidden border-t border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 p-4 space-y-3 shadow-lg max-h-[85vh] overflow-y-auto">
          <nav className="flex flex-col gap-1">
            ...
            <button
              onClick={() => handleNav("catalog")}
              className={mobileNavItemClass("catalog")}
            >
            ...
            {user && (
              <button
                onClick={() => handleNav("favorites")}
                className={mobileNavItemClass("favorites")}
              >
            ...
            {(isOwner || isVendedor) && (
              <div className="border-t border-neutral-100 dark:border-neutral-800 my-2 pt-2 space-y-1">
                ...
                <button
                  onClick={() => handleNav("orders")}
                  className={mobileNavItemClass("orders")}
                >
                ...
                <button
                  onClick={() => handleNav("customers")}
                  className={mobileNavItemClass("customers")}
                >
                ...
                <button
                  onClick={() => handleNav("showroom")}
                  className={mobileNavItemClass("showroom")}
                >
                ...
                <button
                  onClick={() => handleNav("inventory")}
                  className={mobileNavItemClass("inventory")}
                >
                ...
                <button
                  onClick={() => handleNav("barcodes")}
                  className={mobileNavItemClass("barcodes")}
                >
                ...
                {isOwner && (
                  <button
                    onClick={() => handleNav("config")}
                    className={mobileNavItemClass("config")}
                  >
