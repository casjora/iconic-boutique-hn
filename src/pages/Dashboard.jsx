...
export default function Dashboard() {
  ...
  // 9. NO-MOVEMENT & LOW-STOCK INVENTORY ANALYSIS (KEY INDICATOR)
  const inventoryAlerts = useMemo(() => {
    // Collect sold quantities in selected timeframe per product
    const soldInPeriod = {};
    filteredOrders.filter(o => o.status === 'entregado').forEach(o => {
      (o.items || []).forEach(item => {
    ...
    // Products with stock > 0 but ZERO sales in the period
    const noMovement = products.filter(p => p.stock > 0 && (!soldInPeriod[p.id] || soldInPeriod[p.id] === 0));
    
    // Calculate total capital locked in non-moving stock
    ...
    // Low stock products (stock <= 3 and > 0)
    ...
    <div className="space-y-8 fade-in-up pb-10 max-w-7xl mx-auto w-full">
      ...
      <div className="space-y-6">
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          ...
          {/* Ticket Promedio (AOV) */}
          <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-3xl p-6 shadow-sm flex items-start gap-4">
            <div className="p-3 bg-amber-50 dark:bg-amber-950/60 text-amber-600 dark:text-amber-400 rounded-2xl flex-shrink-0">
              <TrendingUp className="h-6 w-6" />
            </div>
            <div className="space-y-1 overflow-hidden">
              <span className="text-[10px] font-bold text-neutral-400 dark:text-neutral-500 uppercase tracking-widest font-mono">Ticket Promedio (AOV)</span>
              <span className="block text-2xl font-black text-neutral-950 dark:text-neutral-100 font-mono truncate">
          ...
          {/* Owner Profit / Cost */}
          {user?.role === 'owner' && (
            <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-3xl p-6 shadow-sm flex items-start gap-4 ring-2 ring-emerald-500/20">
              <div className="p-3 bg-emerald-100 dark:bg-emerald-950/80 text-emerald-700 dark:text-emerald-300 rounded-2xl flex-shrink-0">
                <DollarSign className="h-6 w-6" />
              </div>
              <div className="space-y-1 overflow-hidden">
                <span className="text-[10px] font-bold text-emerald-800 dark:text-emerald-400 uppercase tracking-widest font-mono">Ganancia Neta</span>
                <span className="block text-2xl font-black text-emerald-700 dark:text-emerald-400 font-mono truncate">
          ...
          {/* Pending Pipeline */}
          <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-3xl p-6 shadow-sm flex items-start gap-4">
            <div className="p-3 bg-indigo-50 dark:bg-indigo-950/60 text-indigo-600 dark:text-indigo-400 rounded-2xl flex-shrink-0">
              <Clock className="h-6 w-6 animate-pulse" />
            </div>
            <div className="space-y-1 overflow-hidden">
              <span className="text-[10px] font-bold text-neutral-400 dark:text-neutral-500 uppercase tracking-widest font-mono">Ventas Pendientes</span>
              <span className="block text-2xl font-black text-indigo-950 dark:text-indigo-300 font-mono truncate">
                L. {salesStats.pendingRevenue.toLocaleString()}
              </span>
              <span className="block text-[10px] font-semibold text-indigo-700 dark:text-indigo-400 flex items-center gap-1">
          ...
          <div className="bg-gradient-to-br from-amber-50 to-orange-50 dark:from-amber-950/40 dark:to-orange-950/40 border border-amber-200 dark:border-amber-900/60 rounded-3xl p-6 shadow-sm flex items-start gap-4 relative overflow-hidden">
            <div className="absolute right-[-10px] bottom-[-10px] text-amber-200/40 dark:text-amber-500/10 opacity-40">
              <Trophy className="h-24 w-24" />
            </div>
            <div className="p-3 bg-amber-400 text-white dark:text-neutral-950 rounded-2xl shadow-sm z-10 flex-shrink-0">
              <Crown className="h-6 w-6" />
            </div>
            <div className="space-y-1 z-10 overflow-hidden">
              <span className="text-[10px] font-bold text-amber-800 dark:text-amber-300 uppercase tracking-widest font-mono flex items-center gap-1">
                <Sparkles className="h-3 w-3" /> Cliente Destacado
              </span>
              {clienteDelMes ? (
                <>
                  <span className="block text-sm font-black text-neutral-900 dark:text-neutral-100 truncate">
                    {clienteDelMes.name}
                  </span>
                  <span className="block text-[10px] font-semibold text-amber-900 dark:text-amber-200">
                ...
                <>
                  <span className="block text-sm font-black text-neutral-600 dark:text-neutral-400">
                    Sin Datos
                  </span>
        ...
        <div className="grid gap-6 md:grid-cols-2">
          ...
          <div className="bg-amber-50/60 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-900/50 rounded-3xl p-6 shadow-sm flex flex-col justify-between space-y-4">
            <div className="flex items-start justify-between gap-2">
              <div className="space-y-1">
                <span className="text-[10px] font-bold text-amber-800 dark:text-amber-300 uppercase tracking-widest font-mono flex items-center gap-1">
                  <AlertTriangle className="h-3.5 w-3.5 text-amber-600 dark:text-amber-400" /> Alerta de Inventario Sin Movimiento
                </span>
                <h4 className="font-display font-bold text-neutral-900 dark:text-neutral-100 text-xs">
                  Productos con Stock Físico sin Ventas en este Período
                </h4>
              </div>
              <span className="bg-amber-100 dark:bg-amber-900/60 text-amber-900 dark:text-amber-200 border border-amber-200 dark:border-amber-800 font-mono font-black text-xs px-2.5 py-1 rounded-xl flex-shrink-0">
                {inventoryAlerts.noMovementCount} frascos
              </span>
            </div>

            <div className="bg-white/80 dark:bg-neutral-900/80 border border-amber-200/80 dark:border-amber-900/50 rounded-2xl p-4 space-y-2">
              ...
              <div className="flex justify-between items-center text-xs border-t border-amber-100 dark:border-amber-900/40 pt-1.5">
            ...
            <div className="space-y-2">
              ...
              >
                ...
                {isNoMovementOpen ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />}
              </button>
                    ...
                    inventoryAlerts.noMovement.map((p) => (
                      <div key={p.id} className="py-1.5 flex justify-between items-center">
                        <div className="truncate pr-2">
                          ...
                          <span className="text-[10px] text-neutral-500 dark:text-neutral-400">Costo: L. {p.cost} | Venta: L. {p.pricePublic}</span>
                        </div>
                        <span className="bg-amber-50 dark:bg-amber-950/60 text-amber-800 dark:text-amber-300 font-mono font-black px-2 py-0.5 rounded-md border border-amber-200 dark:border-amber-800 flex-shrink-0">
                          {p.stock} u. en stock
                        </span>
                      </div>
                    ))
          ...
          <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-3xl p-6 shadow-sm flex flex-col justify-between space-y-4">
            <div className="flex items-start justify-between gap-2 border-b border-neutral-100 dark:border-neutral-800 pb-3">
              <div className="space-y-1">
                <span className="text-[10px] font-bold text-rose-600 dark:text-rose-400 uppercase tracking-widest font-mono flex items-center gap-1">
                  <Flame className="h-3.5 w-3.5 text-rose-500 dark:text-rose-400 animate-bounce" /> Alerta de Reabastecimiento
                </span>
                <h4 className="font-display font-bold text-neutral-900 dark:text-neutral-100 text-xs">
                  Fragancias por Agotarse (Stock ≤ 3 unidades)
                </h4>
              </div>
              <span className="bg-rose-50 dark:bg-rose-950/60 text-rose-700 dark:text-rose-300 border border-rose-100 dark:border-rose-900 font-mono font-black text-xs px-2.5 py-1 rounded-xl flex-shrink-0">
            ...
            <div className="space-y-2 flex-1 overflow-y-auto max-h-44">
              {inventoryAlerts.lowStock.length === 0 ? (
                ...
                inventoryAlerts.lowStock.slice(0, 5).map((p) => (
                  <div key={p.id} className="flex justify-between items-center text-xs p-2 bg-neutral-50 dark:bg-neutral-800/60 rounded-xl border border-neutral-100 dark:border-neutral-800">
                    <div className="truncate pr-2">
                      <span className="font-bold text-neutral-900 dark:text-neutral-100 block truncate">{p.brand} - {p.name}</span>
                      <span className="text-[10px] text-neutral-400 dark:text-neutral-500">{p.size || 'N/A'}</span>
                    </div>
                    <span className="bg-rose-100 dark:bg-rose-950/80 text-rose-800 dark:text-rose-300 font-mono font-black px-2 py-0.5 rounded-md text-[10px] flex-shrink-0">
                      ¡Quedan {p.stock} u.!
                    </span>
                  </div>
                ))
                  ...
                  return (
                    <div key={idx} className="space-y-1">
                      <div className="flex justify-between text-[11px] font-bold">
                        <span className="text-neutral-700 dark:text-neutral-200">{item.category}</span>
                        <span className="text-neutral-500 dark:text-neutral-400 font-mono">L. {item.revenue.toLocaleString()} ({item.quantity} u.)</span>
                      </div>
        ...
        <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-3xl overflow-hidden shadow-sm">
          <button 
            ...
            className="w-full px-6 py-4 bg-neutral-50 hover:bg-neutral-100/60 dark:bg-neutral-800/50 dark:hover:bg-neutral-800 border-b border-neutral-100 dark:border-neutral-800 flex items-center justify-between transition-colors cursor-pointer text-left"
          >
            <div className="flex items-center gap-2">
              <Calendar className="h-4 w-4 text-emerald-600 dark:text-emerald-400" />
              <span className="text-sm font-bold text-neutral-900 dark:text-neutral-100 font-display">Histórico Mensual del Año (Desglose de Caja)</span>
              <span className="text-[10px] text-neutral-400 dark:text-neutral-400 bg-white dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 px-2 py-0.5 rounded-lg font-mono">
      ...
      <div className="space-y-6 pt-5 border-t border-neutral-200 dark:border-neutral-800">
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <div className="bg-neutral-50 dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-3xl p-6 shadow-sm flex flex-col justify-between">
            ...
            <div className="text-center border-t border-neutral-100 dark:border-neutral-800 pt-3 mt-3">
        ...
        <div className="grid gap-6 md:grid-cols-2">
          ...
          <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-3xl p-6 shadow-sm space-y-4">
            <h3 className="font-display font-bold text-neutral-900 dark:text-neutral-100 text-sm border-b border-neutral-100 dark:border-neutral-800 pb-3 flex items-center gap-1.5">
              <Tags className="h-4 w-4 text-indigo-500 dark:text-indigo-400" /> Marcas con Mayor Volumen en Stock
            </h3>
                  ...
                  return (
                    <div key={idx} className="space-y-1">
                      <div className="flex justify-between text-xs font-semibold">
                        <span className="text-neutral-800 dark:text-neutral-200">{item.brand}</span>
                        <span className="text-neutral-500 dark:text-neutral-400 font-mono">{item.stock} u. ({percentage}%)</span>
                      </div>
          ...
          <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-3xl p-6 shadow-sm space-y-4">
            <h3 className="font-display font-bold text-neutral-900 dark:text-neutral-100 text-sm border-b border-neutral-100 dark:border-neutral-800 pb-3 flex items-center gap-1.5">
              <FileSpreadsheet className="h-4 w-4 text-amber-500 dark:text-amber-400" /> Distribución de Stock por Categoría
            </h3>
              ...
              {categoryBreakdown.length === 0 ? (
                ...
                categoryBreakdown.map((item, idx) => {
                  ...
                  return (
                    <div key={idx} className="space-y-1">
                      <div className="flex justify-between text-xs font-semibold">
                        <span className="text-neutral-800 dark:text-neutral-200">{item.category}</span>
                        <span className="text-neutral-500 dark:text-neutral-400 font-mono">{item.stock} u. ({percentage}%)</span>
                      </div>
                      <div className="h-2 w-full bg-neutral-100 dark:bg-neutral-800 rounded-full overflow-hidden">
                        <div 
        ...
        <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-3xl shadow-sm overflow-hidden p-6 space-y-4">
          <div>
            <h3 className="font-display font-bold text-neutral-900 dark:text-neutral-100 text-sm flex items-center gap-1.5 border-b border-neutral-100 dark:border-neutral-800 pb-3">
              <Tags className="h-4 w-4 text-emerald-600 dark:text-emerald-400" /> Catálogo Detallado de Marcas (Desglose hasta Perfume)
            </h3>
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mt-1">
              ...
              <span className="text-[10px] font-bold text-neutral-400 dark:text-neutral-400 uppercase tracking-widest bg-neutral-100 dark:bg-neutral-800 px-2.5 py-1 rounded-md">
              ...
              return (
                ...
                >
                  ...
                  <button
                    ...
                    className="w-full px-5 py-3.5 bg-neutral-50 hover:bg-neutral-100/60 dark:bg-neutral-800/60 dark:hover:bg-neutral-800 flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-left transition-colors cursor-pointer"
                  >
                    <div>
                      <span className="text-xs font-black text-neutral-900 dark:text-neutral-100 font-mono tracking-wider uppercase">
                        {brandInfo.brandName}
                      </span>
                      <div className="flex flex-wrap items-center gap-2 text-[10px] text-neutral-500 dark:text-neutral-400 mt-0.5 font-mono">
                    ...
                    <div className="flex items-center gap-2 self-end sm:self-center">
                      ...
                      {isExpanded ? <ChevronUp className="h-4 w-4 text-neutral-500 dark:text-neutral-400" /> : <ChevronDown className="h-4 w-4 text-neutral-500 dark:text-neutral-400" />}
                    </div>
                  </button>

                  {/* Perfumes Sub-Table Expanded Section */}
                  {isExpanded && (
                    <div className="p-4 bg-white dark:bg-neutral-900 border-t border-neutral-100 dark:border-neutral-800 overflow-x-auto">
                      <table className="w-full text-left text-xs divide-y divide-neutral-100 dark:divide-neutral-800">
